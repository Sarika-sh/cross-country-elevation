...
async function loadElevation(route) {
  ...
  let coords = [];
  json.docs[0].features.forEach(feature => {
  ...
  for (let i = 0; i < coords.length; i++) {
    ...
    // For Melbourne, we don't have elevation, so just set it to 0 or skip
    let elevation = 0;

    // If the route is not Melbourne, check for elevation data
    if (route.name !== "Melbourne") {
      // Check if elevation data is available in the current coordinate
      if (coord.length >= 3) {
        elevation = coord[2]; // Use the 3rd element (elevation) if available
      }
    }

    // Calculate distances using haversine formula if not Melbourne
    if (i > 0 && Array.isArray(coords[i - 1]) && coords[i - 1].length >= 2) {
  ...
  const points = distances.map((d, i) => {
    ...
    const y = route.name === "Melbourne" 
      ? margin.top + plotHeight * 0.5 // Flat line for Melbourne (since there's no elevation)
      : margin.top + plotHeight * (1 - (elev - minElev) / elevRange);
    return [x, y];
  });
  ...
  svg.insertBefore(shadedPath, path);
